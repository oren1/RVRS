...
public class AssetReverseSession {
    ...
    public required init(asset: AVAsset, outputFileURL: URL) async {
        self.asset = asset
        ...
        if let videoTrack = try? await asset.loadTracks(withMediaType: .video).first,
           let preferredTransform = try? await videoTrack.load(.preferredTransform) {
            ...
            transform = VideoHelper.getVideoTransform(orientation: orientation)
        }
        else {
            transform = .identity
        }
...
//            }
    ...
    private func prepareReaderAndWriter() throws {
        ...
        assetWriterInput.transform = self.transform
